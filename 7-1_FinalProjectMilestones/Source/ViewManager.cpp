	...
	ShaderManager *pShaderManager)
{
	...
	g_pCamera->Zoom = 80;
	// added default movespeed and sensitivity to be more usable
	g_pCamera->MovementSpeed = 10;
	g_pCamera->MouseSensitivity = .075;
}
...
GLFWwindow* ViewManager::CreateDisplayWindow(const char* windowTitle)
{
	...
	// tell GLFW to capture all mouse events (lock cursor to center and hide it to allow continuous turning)
...
void ViewManager::ProcessKeyboardEvents()
{
	...
	//// process camera slowing alternatively
	//if (glfwGetKey(m_pWindow, GLFW_KEY_LEFT_CONTROL) == GLFW_PRESS)
	//{
	//	g_pCamera->MovementSpeed = 5;
	//}
	//if (glfwGetKey(m_pWindow, GLFW_KEY_LEFT_CONTROL) == GLFW_RELEASE)
	//{
	//	g_pCamera->MovementSpeed = 20;
	//}

	// Process orthographic and perspective toggle
	if (glfwGetKey(m_pWindow, GLFW_KEY_P) == GLFW_PRESS)
	{
		bOrthographicProjection = false; // 3D
	}
	if (glfwGetKey(m_pWindow, GLFW_KEY_O) == GLFW_PRESS)
	{
		bOrthographicProjection = true; // 2D
	}
}
